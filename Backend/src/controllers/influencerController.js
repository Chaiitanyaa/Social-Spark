...
export const findInfluencers = async (req, res) => {
    ...
    if (!description) return res.status(400).json({ message: 'Product description required' });
    ...
    try {
        ...
        console.log(`Found influencers:`, influencers.map(i => i.name));

        // Step 3: Find lookalikes (5 API calls)
        const lookalikes = await fetchLookalikeInfluencers(influencers);
        console.log(`Found lookalikes:`, lookalikes.map(i => i.name));
